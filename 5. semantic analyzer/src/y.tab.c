...
{
       0,   174,   174,   177,   178,   181,   182,   185,   185,   187,
     187,   191,   192,   195,   196,   199,   202,   203,   204,   205,
     208,   209,   210,   213,   214,   217,   218,   221,   222,   225,
     226,   229,   230,   233,   234,   235,   238,   239,   238,   241,
     242,   241,   244,   247,   248,   251,   252,   255,   258,   259,
     262,   265,   265,   267,   267,   269,   272,   273,   276,   277,
     277,   281,   282,   285,   286,   289,   290,   291,   292,   292,
     296,   297,   300,   301,   304,   305,   308,   309,   312,   313,
     316,   317,   318,   321,   322,   323,   324,   325,   328,   329,
     332,   333,   336,   337,   338,   339,   340,   341,   344,   345,
     348,   348,   354,   355,   358,   359,   360,   363,   364,   365,
     368,   371,   372,   375,   376,   377,   380,   381,   384,   385,
     388,   389,   392,   395,   398,   401,   402,   405,   408,   409,
     412,   413,   416,   419,   422,   425,   426,   427,   430,   431,
     432,   433,   434,   437,   440,   441,   442,   445,   446,   447,
     448,   451,   452,   455,   456,   457,   458,   459,   460,   461,
...
void setTypeSize(A_TYPE*, int); //호출된 적이 없음...
...
float atof(A_NODE* node) {return 0.0;}////////////////////////////// 구현 필요할듯 return 0.0함
...
A_TYPE* sem_expression(A_NODE* node) {
    ...
    switch(node->name) {
        ...
        case N_EXP_FLOAT_CONST:
            lit.type=float_type;
            lit.value.f=atof(node->clink); //atof 미구현
        ...
        case N_EXP_FUNCTION_CALL:
            ...
            node->llink=convertUsualBinaryConversion(node->llink);
            t=node->llink->type;
...
int sem_statement(A_NODE* node, int addr, A_TYPE* ret, BOOLEAN sw, BOOLEAN brk, BOOLEAN cnt) {
    ...
    switch(node->name) {
        ...
        case N_STMT_IF_ELSE:
            ...
            int i=sem_statement(node->rlink, addr, ret, FALSE, brk, cnt);
            if(local_size<i)
                local_size=i;
            break;
        case N_STMT_SWITCH:
...
int sem_declaration(A_ID* id, int addr) {
    ...
    switch(id->kind) {
        ...
        case ID_TYPE:
            ...
            break;
        default:
            semantic_error(89, id->line); ////////////////////////////// 마지막 파라미터 제거
            break;
    }
    return (size);
}
...
void semantic_error(int i, int ll) {////////////////////////////// 마지막 파라미터 char* 제거, 여태 호출 시 파라미터 정수 2개만 줌
    semantic_err++;
    ...
    //errors in expression
        ...
        case 18:
            ...
            break;
        case 19:
            printf("illegal indentifier in constant expression\n");
            break;
        case 21:
